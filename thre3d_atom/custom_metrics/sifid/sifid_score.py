...
parser.add_argument(
    "--images_suffix", default="jpg", type=str, help="image file suffix"
)


def get_activations(
    files, model, batch_size=1, dims=64, cuda=False, verbose=False, resize_image=False
):
    ...
    for i in tqdm(range(n_batches)):
        ...
        images = images[:, :, :, 0:3]
        if images.shape[1] == 270 and images.shape[2] == 480:
            # This happens for the blocks images
        ...
        if images.shape[1] == 256 and images.shape[2] == 256:
            # resize image to 512 x 512
            # This happens for chalk images
            image = images[0]
            img = Image.fromarray((image * 255).astype(np.uint8))
        ...
        if resize_image:
            # resize the input image to size 299 x 299 so that we only get one activation vector
            # per image
            image = images[0]
            img = Image.fromarray((image * 255).astype(np.uint8))
...
def calculate_sifid_given_paths(
    path_list_1, path_list_2, batch_size, cuda, dims
):  # suffix):
    """Calculates the SIFID of two lists of paths"""
    assert len(path_list_1) == len(
        path_list_2
    ), f"The two provided path-lists are incompatible"
    ...
    for i in range(len(path_list_2)):
        m1, s1 = calculate_activation_statistics(
            [path_list_1[i]], model, batch_size, dims, cuda
        )
        m2, s2 = calculate_activation_statistics(
            [path_list_2[i]], model, batch_size, dims, cuda
        )
        fid_values.append(calculate_frechet_distance(m1, s1, m2, s2))
        file_num1 = path_list_1[i].name
        file_num2 = path_list_2[i].name
...
if __name__ == "__main__":
    ...
    sifid_values = calculate_sifid_given_paths(
        path1, path2, 1, args.gpu != "", 64, suffix
    )
